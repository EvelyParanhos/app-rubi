...
public interface TransactionRepository extends JpaRepository<Transaction, UUID> {
    ...
    @Query("SELECT COALESCE(SUM(CASE WHEN t.type = com.rubi.core.domain.TransactionType.CREDIT OR t.type = com.rubi.core.domain.TransactionType.OPENING_BALANCE THEN t.amount ELSE -t.amount END), 0) FROM Transaction t WHERE t.account.id = :accountId AND t.status = com.rubi.core.domain.TransactionStatus.CONFIRMED")
    BigDecimal calculateBalance(@Param("accountId") UUID accountId);

    @Query("SELECT COALESCE(SUM(CASE WHEN t.type = com.rubi.core.domain.TransactionType.DEBIT THEN t.amount ELSE -t.amount END), 0) FROM Transaction t WHERE t.invoice.id = :invoiceId AND t.status = com.rubi.core.domain.TransactionStatus.CONFIRMED")
